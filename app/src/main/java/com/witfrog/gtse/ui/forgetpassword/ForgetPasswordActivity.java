...
public class ForgetPasswordActivity extends BaseActivity<ForgetPasswordPresenter> implements ForgetPasswordView {
    ...
    public void retrievePwd() {
        sendMessage(Constant.MSG_MODIFY_PASSWORD_SUCCESS, etPhoneNumber.getText().toString());
        finish();
    }

    @OnClick({R.id.tv_login, R.id.btn_send, R.id.iv_eye, R.id.tv_confirm, R.id.layout_fp})
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_confirm: {
                ...
                if (StringUtils.isSpace(phone)) {
                    ...
                    mPresenter.modifyPassword(phone, code, password);
                }
            }
            break;
            default:
